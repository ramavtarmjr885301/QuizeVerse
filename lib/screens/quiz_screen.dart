...
//   bool _hasNavigatedToResult = false; // FIX: guard against duplicate navigation
...
//                   // Timer — FIX: keyed by currentIndex so it resets fresh per question
//                   TimerWidget(
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  int? _selectedOptionIndex;
  bool _isTimerFinished = false;
  bool _hasNavigatedToResult = false; // guard against duplicate navigation
                  ...
                  Expanded(
                    child: QuestionCard(
                      ...
                      onOptionSelected: (index) async {
                        ...
                        // Wait so the user can SEE the result on the CURRENT question
                        // before we advance — this is what stops the "next answer shows early" bug
                        await Future.delayed(const Duration(milliseconds: 800));

                        if (mounted) {
                          quizProvider
                              .moveToNextQuestion(); // NOW advance to next question
                          setState(() {
                            _isAnswered = false;
                            _selectedOptionIndex = null;
                            _isTimerFinished = false;
